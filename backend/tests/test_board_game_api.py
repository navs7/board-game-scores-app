...
class TestGameLifecycle:
    # ---- shared helpers ----
    @staticmethod
    def _start_highest(admin_headers, names=("Alice", "Bob", "Carol")):
        requests.post(f"{API}/game/abandon", headers=admin_headers)
        body = {"game_name": "TEST_HighGame", "ranking_order": "highest", "players": list(names)}
        r = requests.post(f"{API}/game/start", json=body, headers=admin_headers)
        assert r.status_code == 200, r.text
        return {p["name"]: p["key"] for p in r.json()["players"]}

    @staticmethod
    def _submit(admin_headers, key, score):
        requests.post(f"{API}/game/submit-score", json={"player_key": key, "score": score}, headers=admin_headers)

    def test_start_creates_three_players(self, admin_headers):
        keys = self._start_highest(admin_headers)
        cur = requests.get(f"{API}/game/current").json()
        assert len(cur["players"]) == 3
        assert set(keys.keys()) == {"Alice", "Bob", "Carol"}

    def test_submit_score_accumulates_total_and_scores(self, admin_headers):
        keys = self._start_highest(admin_headers)
        self._submit(admin_headers, keys["Alice"], 10)
        self._submit(admin_headers, keys["Alice"], 5)
        cur = requests.get(f"{API}/game/current").json()
        alice = next(p for p in cur["players"] if p["name"] == "Alice")
        assert alice["totalScore"] == 15
        assert alice["scores"] == [10, 5]

    def test_undo_score_removes_last(self, admin_headers):
        keys = self._start_highest(admin_headers)
        self._submit(admin_headers, keys["Bob"], 25)
        self._submit(admin_headers, keys["Bob"], 30)
    ...
    def test_end_game_saves_history_and_player_records(self, admin_headers):
        keys = self._start_highest(admin_headers)
        for name, score in [("Alice", 15), ("Bob", 55), ("Carol", 35)]:
            self._submit(admin_headers, keys[name], score)
        end = requests.post(f"{API}/game/end", headers=admin_headers)
        assert end.status_code == 200
        assert end.json()["winner_label"] == "Bob"
        # current cleared
